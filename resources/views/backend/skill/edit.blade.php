...
@section('content')
    <div class="w-4xl mx-auto bg-white p-8 rounded-lg shadow-md">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-700">Edit Skill</h1>
        ...
        <!-- type -->
            <div class="mb-4">
                ...
                <input type="text" name="type" id="type" class="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ old('type', $skill->type) }}" required>
                @error('type')
        ...
        <!-- Title -->
            <div class="mb-4">
                ...
                <input type="text" name="title" id="title" class="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ old('title', $skill->title) }}" required>
                @error('title')
                <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="6" class="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $skill->description) }}</textarea>
                @error('description')
                <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="progress" id="progress" class="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ old('progress', $skill->progress) }}">
                @error('progress')
                <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <div class="mt-2">
                    @if ($skill->image)
                        <img src="{{ asset('storage/' . $skill->image) }}" alt="skill Image" class="h-20 rounded-lg shadow">
                    @endif
                </div>
                @error('image')
                <span class="text-red-500 text-sm">{{ $message }}</span>
...
{{--                    <option value="0" {{ old('status', $skill->status) == '0' ? 'selected' : '' }}>Inactive</option>--}}
                    <option value="1" {{ old('status', $skill->status) == 'active' ? 'selected' : '' }}>Active</option>
                    <option value="0" {{ old('status', $skill->status) == '' ? 'selected' : '' }}>Inactive</option>
                </select>
                @error('status')
                <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
...
{{--                <input type="text" name="slug" id="slug" class="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ old('slug', $skill->slug) }}">--}}
{{--                @error('slug')--}}
{{--                <span class="text-red-500 text-sm">{{ $message }}</span>--}}
{{--                @enderror--}}
{{--            </div>--}}
